import { IBasket, OrderShowEventSubscription, OrderSubmitEventTrigger, PaymentMethod } from "../../types";
...
export class OrderView {
    ...
    constructor(private modalForm: ModalForm, private orderShowEventSubscription: OrderShowEventSubscription, private orderSubmitEventTrigger: OrderSubmitEventTrigger) {
    ...
    private showOrderStepOneForm(basket: IBasket): void {
        const formFragment = this.orderTemplate.content.cloneNode(true) as DocumentFragment;
        const formElement = formFragment.firstElementChild as HTMLFormElement;

        const buttonCardPayment = ensureElement<HTMLButtonElement>('button[name="card"]', formElement);
        const buttonCashPayment = ensureElement<HTMLButtonElement>('button[name="cash"]', formElement);
        const addressInput = ensureElement<HTMLInputElement>('input[name="address"]', formElement);
        const nextButton = ensureElement<HTMLButtonElement>('button[type="submit"]', formElement);
        const errorSpan = ensureElement<HTMLSpanElement>('.form__errors', formElement);
    ...
    private showOrderStepTwoForm(basket: IBasket): void {
        ...
        const formFragment = this.contactsTemplate.content.cloneNode(true) as DocumentFragment;
        const formElement = formFragment.firstElementChild as HTMLFormElement;

        const emailInput = ensureElement<HTMLInputElement>('input[name="email"]', formElement);
        const phoneInput = ensureElement<HTMLInputElement>('input[name="phone"]', formElement);
        const submitButton = ensureElement<HTMLButtonElement>('button[type="submit"]', formElement);
        const errorSpan = ensureElement<HTMLSpanElement>('.form__errors', formElement);
        ...
        formElement.addEventListener('submit', (event) => {
            event.preventDefault();
            updateModel();
            const orderModel = new OrderModel(basket, this.shippingAndPaymentInfoModel, this.customerInfoModel)
