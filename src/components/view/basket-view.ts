...
export class BasketView {
	...
	private showBasket(basket: IBasket): void {
		...
		basket.products.forEach((product, index) => {
			const itemFragment =
				this.cardTemplate.content.firstElementChild!.cloneNode(
					true
				) as HTMLElement;

			const itemIndex = ensureElement<HTMLSpanElement>(
				'.basket__item-index',
				itemFragment
			);
			const itemTitle = ensureElement<HTMLSpanElement>(
				'.card__title',
				itemFragment
			);
			const itemPrice = ensureElement<HTMLSpanElement>(
				'.card__price',
				itemFragment
			);
			const itemDeleteButton = ensureElement<HTMLButtonElement>(
				'.basket__item-delete',
				itemFragment
			);
			...
			itemDeleteButton.addEventListener('click', () =>
				this.deleteBasketItem(product, itemFragment)
			);
